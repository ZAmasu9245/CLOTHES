...
local function OpenCart(Panel)
	...
	head.Paint = function()
		draw.SimpleText( CLOTHESMOD.Config.Sentences[17][CLOTHESMOD.Config.Lang].." - "..CLOTHESMOD.Config.Sentences[18][CLOTHESMOD.Config.Lang].." : "..ttprice..CLOTHESMOD.Config.MoneyUnit, "Bariol25",w/2,60/2,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
	end
	...
	for name,tables in pairs( cart ) do
		...
		dCart.Paint = function(pnl, wi,he)
			...
			draw.SimpleText( string.sub(name,0,string.len(name)-1).." - "..tables.price..CLOTHESMOD.Config.MoneyUnit, "Bariol20",wi/2,he/2,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, 1 )
		end
		...
		DButton.Paint = function(p, w, h)
			...
			draw.SimpleText( "X", "Bariol20",w/2,h/2,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, 1 )
...
local function OpenList( Panel, cat, CP )
	...
	DPanel.Paint = function()
		draw.SimpleText( CLOTHESMOD.Config.Sentences[20][CLOTHESMOD.Config.Lang], "Bariol25",w/2,30,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
		if err ~= "" then
			draw.SimpleText( CLOTHESMOD.Config.Sentences[10][CLOTHESMOD.Config.Lang]..": "..err, "Bariol15",w/2,60,Color( 255, 0, 0, 255 ), TEXT_ALIGN_CENTER )
		end
	end
	...
	for name, tables in pairs ( list ) do
		...
		DButton2.Paint = function(pnl, w, h )
			...
			draw.SimpleText( name, "Bariol18",w/2,0,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, 0 )
			draw.RoundedBox(0,0,17,w,2, Color(255,255,255,255))

			draw.SimpleText( tables.price..CLOTHESMOD.Config.MoneyUnit, "Bariol15",w/2,h/2-6,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, 0 )
			draw.SimpleText( CLOTHESMOD.Config.Sentences[21][CLOTHESMOD.Config.Lang], "Bariol15",w/2,h/2+6,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, 0 )
...
OpenCharacterPage = function( Frame, sizex, sizey, ent )
	...
	DButton1.Paint = function(pnl, w, h )
		...
		draw.SimpleText( CLOTHESMOD.Config.Sentences[23][CLOTHESMOD.Config.Lang], "Bariol25",(panelx/2-2.5)/2,30/2-1,Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	...
	DPanelMenu.Paint = function(pnl, w, h)
		...
		draw.SimpleText("Categories", "Bariol25", w/2,10,Color(255,255,255),1)
		
		local nbc2 = 0
		
		for k,v in pairs( categories ) do
			
			surface.SetFont("Bariol18")
			
			local x = surface.GetTextSize(k)
			if x + 20 + 10 >= 80+50 then
				draw.SimpleText(k, "Bariol14", 20+10,48+30*nbc2+10,Color(255,255,255),0,1)
			else
				draw.SimpleText(k, "Bariol18", 20+10,48+30*nbc2+10,Color(255,255,255),0,1)
			end
			nbc2 = nbc2 + 1
